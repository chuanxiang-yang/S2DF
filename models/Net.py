...
class Network(nn.Module):
    def __init__(self, in_dim=3, decoder_hidden_dim=256, nl='sine',
        ...
        if neuron_type == 'linear':
            ...
            self.decoder.fc_block = QuaNet(d_in=in_dim, nl=nl, n_layers=decoder_n_hidden_layers,
    ...
    def forward(self, non_mnfld_pnts=None, mnfld_pnts=None, near_points=None):
        ...
        if near_points is not None:
            near_points_pred = self.decoder(near_points.view(-1, near_points.shape[-1])).reshape(batch_size, -1)
        return {"manifold_pnts_pred": manifold_pnts_pred,"nonmanifold_pnts_pred": nonmanifold_pnts_pred,"near_points_pred": near_points_pred}
